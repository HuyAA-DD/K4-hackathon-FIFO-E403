...
const { SYSTEM_PROMPT, routeQuestion } = require("./src/chat-policy");
const { respond, classifyRoute, getWebCitations } = require("./src/providers/openai");
...
async function handleChat(req, res) {
  ...
  const response = await respond({
    useWebSearch: true,
    input: `${SYSTEM_PROMPT}\n\nROUTE: WEB\nBối cảnh bài học/slide liên quan:\n${localContext}\n\nCÂU HỎI: ${question}\n\nBắt buộc dùng Web Search trước khi trả lời, kể cả khi slide đã có nội dung nền. Tổng hợp tối đa 5 nguồn mạnh, ưu tiên tài liệu chính thức hoặc học thuật. Không tự ghi URL/citation trong phần văn bản; hệ thống sẽ hiển thị citation từ metadata.`
  });
  const citations = getWebCitations(response.annotations);
  ...
  return sendJson(res, 200, { route: "external", answer: response.text, citations, topic: decision.query });
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === "/api/search") {
      ...
      const results = searchPages(query, readIndex().pages, 3);
